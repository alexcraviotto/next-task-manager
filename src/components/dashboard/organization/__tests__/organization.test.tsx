...
describe('Organization Creation and Listing Flow', () => {
  ...
  it('creates an organization and then displays it in the organizations page', async () => {
    ...
    const nameInput = createOrgModal.getByPlaceholderText('Introduzca el nombre de la organización');
    const effortLimitInput = createOrgModal.getByLabelText('Esfuerzo Límite:');
    const submitButton = createOrgModal.getByRole('button', { name: /Crear organización/i });
    ...
    expect(mockRouter.push).toHaveBeenCalledWith('/dashboard/organization/org123');
  });
});
